        ...
        cy.wrap(documentoExterno.body).should('be.visible').then(() => {
          ...
          cy.wrap(documentoExterno.body).within(() => {
            ...
            cy.get('.sy-card-group-item').each(($card, index, $list) => {
              ...
              cy.wait(3000)
              cy.get('body > sy-root > div > sy-draft-edit > div.sy-object-edit-container.view-container > div > div.sy-form-row > div.sy-central-column > form > sy-fields > div > div > div.fields-container.sy-grid-col-md.ng-star-inserted > sy-field > fieldset > div > sy-input-option-values > div > p-selectbutton > div > sy-tooltip-card > span > button').click({force:true});
